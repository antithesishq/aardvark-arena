...
- **Game Servers** -- each runs up to a configurable number of concurrent games.
...
go run ./cmd/matchmaker -addr=:8080

# Terminal 2: start a game server
go run ./cmd/gameserver -addr=:8081 -self-url=http://localhost:8081 \
...
go run ./cmd/swarm -n 7 -matchmaker=http://localhost:8080
```
...
internal/
  ...
  game/             # Game implementations (tictactoe, connect4, battleship)
