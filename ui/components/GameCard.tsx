...
function turnCount(state: GameState | null, game: string): number {
  ...
  if (game === "battleship") {
    const attacks = shared?.Attacks as
      | { P1?: Record<string, number>; P2?: Record<string, number> }
      | undefined;
    if (!attacks) {
      return 0;
    }
    return (
      Object.keys(attacks.P1 ?? {}).length +
      Object.keys(attacks.P2 ?? {}).length
    );
  }
  return 0;
}
...
export function GameCard({ session, serverUrl, elapsedSeconds }: Props) {
  ...
  async function handleCancel() {
    ...
    try {
      ...
      if (!res.ok) {
        setCancelError(`Error ${res.status}`);
      }
    } catch {
  ...
  return (
    <div
      ...
      className="bg-zinc-900/20 border border-zinc-800 rounded backdrop-blur-sm p-3 flex flex-col gap-2 w-full aspect-square min-h-0 overflow-hidden"
    >
      {/* Header */}
      <div className="flex items-center justify-between">
        <span className="text-xs text-zinc-400" style={mono}>
          {shortId4(session.session_id)}
        </span>
        ...
        {isFinished ? (
          <div className="flex flex-col items-center gap-1 py-4">
            ...
            <span className="text-xs text-zinc-500" style={mono}>
              {turnCount(gameState, session.game)} turns ·{" "}
          ...
          <TicTacToeBoard
            shared={
              gameState.Shared as Parameters<typeof TicTacToeBoard>[0]["shared"]
            }
          />
        ) : session.game === "connect4" ? (
          <Connect4Board
            shared={
              gameState.Shared as Parameters<typeof Connect4Board>[0]["shared"]
            }
          />
        ) : session.game === "battleship" ? (
          <BattleshipBoard
            shared={
              gameState.Shared as Parameters<
                typeof BattleshipBoard
              >[0]["shared"]
            }
          />
        ) : null}
      ...
      <div className="flex flex-col gap-1">
          ...
          {isFinished ? (
            ...
            <>
              <span className="text-xs text-zinc-400 tabular-nums" style={mono}>
                ⏱ {fmtSeconds(elapsedSeconds)} · Turn{" "}
                {turnCount(gameState, session.game)}
              </span>
              <Button
                size="sm"
                variant="destructive"
        ...
        {cancelError && (
          <span className="text-xs text-red-400" style={mono}>
            {cancelError}
          </span>
        )}
      </div>
