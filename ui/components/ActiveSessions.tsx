...
function CancelButton({
  ...
  return (
    <div className="flex flex-col items-end gap-0.5">
      ...
      {error && (
        <span className="text-[10px] text-red-400" style={mono}>
          {error}
        </span>
      )}
    </div>
...
function CancelAllButton({
  ...
  async function handleCancelAll() {
    ...
    await Promise.allSettled(
      sessions.map((s) => cancelSessionViaMatchmaker(s.session_id)),
    );
    setCancelling(false);
    onRefresh?.();
  }
  ...
  return (
    <div className="bg-zinc-900/20 border border-zinc-800 rounded backdrop-blur-sm py-2 px-3 h-[350px] overflow-y-auto">
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="text-sm font-semibold text-zinc-200" style={geist}>
            Active Sessions
          </div>
          <div className="text-xs text-zinc-400" style={geist}>
            In-progress game sessions
          </div>
        </div>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          ...
          {sorted.map((s) => (
            ...
            >
              <td className="py-2.5" style={mono}>
                <span className="text-zinc-400">{shortId4(s.session_id)}</span>
              </td>
              <td className="py-2.5 text-zinc-300" style={geist}>
                {s.player_ids.map((id) => id.slice(0, 8)).join(" vs ")}
              </td>
              <td className="py-2.5">
                <GameBadgeShort game={s.game} />
              </td>
              <td className="py-2.5 text-zinc-400 text-xs" style={mono}>
                {serverHostname(s.server)}:{new URL(s.server).port}
              </td>
              <td
                className="py-2.5 text-xs text-zinc-300 tabular-nums"
                style={mono}
              >
                {fmtSeconds(
                  Math.floor((now - new Date(s.created_at).getTime()) / 1000),
                )}
              </td>
